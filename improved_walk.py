        ...
        if state["active"] and state["physical_keys_down"]:
            ...
            for target in active_targets:
                for _ in range(state["multiplier"]):
                    subprocess.run(
                        f"xdotool key {target}", shell=True, stderr=subprocess.DEVNULL
                    )
...
def main(stdscr):
    ...
    t = threading.Thread(
        target=lambda: loop.run_until_complete(main_async()), daemon=True
    )
    t.start()
